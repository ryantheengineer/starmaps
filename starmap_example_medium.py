...
def generate_starmap_layers(location_str, when_str):
    """
    ...
    filestring = location_str + when_str
    ...
    # First figure has the stars on it
    fig1, ax1 = plt.subplots(figsize=(chart_size, chart_size), dpi=300)
    ...
    ax1.scatter(stars['x'][bright_stars], stars['y'][bright_stars],
               s=marker_size, color='white', marker='.', linewidths=0, 
               zorder=2)
    
    horizon = Circle((0, 0), radius=1, transform=ax1.transData)
    ...
    plt.savefig('{}stars.svg'.format(filestring))
    plt.close()
    
    # Second figure has the constellation lines on it
    fig2, ax2 = plt.subplots(figsize=(chart_size, chart_size), dpi=300)
    
    border = plt.Circle((0, 0), 1, color='navy', fill=True)
    ax2.add_patch(border)
    # Draw the constellation lines.
    ax2.add_collection(LineCollection(lines_xy, colors='white', linewidths=(0.5)))
    
    horizon = Circle((0, 0), radius=1, transform=ax2.transData)
    for col in ax2.collections:
        col.set_clip_path(horizon)
    ...
    plt.savefig('{}constellations.svg'.format(filestring))
    # plt.show()
    
    
if __name__ == "__main__":
    generate_starmap_layers('Payson, UT', '2015-08-26 22:00')